...
export default function TabeleVechi() {
  ...
  const afiseazaTabel = async () => {
    setLoading(true);
    try {
      const raspuns = await fetch(`http://localhost:4444/api/tabele-vechi-admin/istoric?an=${anSelectat}&luna=${lunaSelectata}`);
      const data = await raspuns.json();
      console.log('Tabel primit:', data); // Debug: vezi dacă vin electricitate și supercom
      setTabel(data);
    } catch (err) {
                  ...
                  <table className="min-w-full border text-sm text-center">
                    <thead className="bg-[#d7ccc8]">
                      <tr>
                        ...
                        <th className="border px-3 py-2">Consum apă (mc)</th>
                        <th className="border px-3 py-2">Apă</th>
                        ...
                        <th className="border px-3 py-2">Întreținere curentă</th>
                    ...
                    <tbody>
                      {tabel.map((rand, idx) => (
                        <tr key={idx} className="border-t hover:bg-gray-100">
                          ...
                          <td className="border px-3 py-2">{rand.apa}</td>
